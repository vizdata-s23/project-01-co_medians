...
ggplot(greg_not_greg) +
  geom_bar(aes(x = year, y = amounts, fill = fct_relevel(factor(is_greg, levels = c("Not Greg Abbott", "Greg Abbott")))),
    position = "stack",
    stat = "identity"
  ) +
  geom_text(
...
ggplot() +
  geom_line(data = plot3_data_no_abbott, mapping = aes(x = factor(year), y = avg_avg_amount, color = position_cat, group = position_cat), size = 2) +
  geom_line(plot3_data_abbott, mapping = aes(x = factor(year), y = avg_amount, group = Politician), size = 2, color = "lightgray", linetype = "dashed") +
  #geom_text_repel(plot3_data_abbott, mapping = aes(x = factor(year), y = avg_amount, label = Politician)) +
  geom_text(plot3_data_no_abbott_text,
            mapping = aes(x = factor(year), y = avg_avg_amount, label = position_cat), hjust = "2022") + 
  geom_text(plot3_data_abbott_text,
            mapping = aes(x = factor(year), y = avg_amount, label = Politician), hjust = "2022") + 
  guides(color = FALSE) + 
  theme_minimal() #+ 
