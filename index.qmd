...
Our initial instinct, when curious about change over time for companies' donations to anti-LGBTQ+ politicians was to create a line plot because they naturally highlight trends over time. Although we had hypothesized about the decrease in total donation amount over time, we wanted to acknowledge that different trends may exist for donations to politicians at different positions and levels of power. Directly comparing total donations to different political positions would not have been beneficial, however, due to there being different numbers of politicians included in various roles within our data. Instead, we thought to calculate each politician's average amount received per year, then average these by position per year. We decided to incorporate three groups, Senators, Representatives, and Other (which includes more specific leadership roles where only one person holds that title per state, such as state Speaker of the House and Caucus Chairman) in order to reduce the number of lines on our plot and also increase the consistency of averages that may not have had many politicians to consider. We also reduced the range of years to focus on just more recent election cycles due to the inclusion of more variety in politicians during the years 2018-2023.

We are interested in examining how the total amount contributed to anti-LGBTQ+ politicians by companies have changed over time. To do this, we created a stacked bar plot with year on the x axis and total amount of contributions on the y axis. The height of each bar indicated that year's total money received by anti-LGBTQ+ politicians across the six states included. Diving further, we wanted to also look at Greg Abbott and how donations to him compares to the overall amount. For the stacked bar plot, we colored by Greg Abbott vs. all other politicians except Abbott. Since Abbott only had data for the years 2019-2022, we focused on those four years in our plot.
