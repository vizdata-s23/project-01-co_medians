...
For this question, we will be using the `static_list` and `contribution_data_all_states` datasets and not `corp_by_politician`. From `static_list`, we will be using the `HRC Business Pledge`, `Pride?`, `Company`, and `Amount Contributed Across States` variables.
...
For our second plot, the density plot, we decided to group companies in four categories: 1) HRC pledge and donates to pride; 2) HRC pledge and doesn't donate to pride; 3) No HRC pledge and donates to pride; and 4) Neither HRC pledge nor donation to pride. For each of the categories, we wanted to look at the distribution of amounts contributed towards anti-LGBTQ+ campaigns. We also included annotations for the number of companies that fell into each category to provide helpful context. We decided to use a density plot because it best captures the shape of the distribution and helps us identify the skew and number of peaks.
...
Looking at the second plot, we can see that the distributions for all four company categories is right-skewed and roughly unimodal. In general, a large proportion of donations for all categories is between 0 and \$200,000. For the category of HRC pledge and donates to pride, we can see that the distribution is very loosely clustered compared to those of the other categories, and there's a wider range of donation amounts. The category of neither (no HRC pledge and doesn't donate to pride) has the tightest distribution of all of the categories. This suggests that companies who are in general anti-LGBTQ+ are donating small amounts towards anti-LGBTQ+ campaigns as opposed to one-time large amounts. However, when interpreting our visualization, we also have to keep in mind that the number of companies that fall under each category is different, thus we included the number of companies as annotations.
...
ggplot() +
  geom_line(data = plot3_data_no_abbott, mapping = aes(x = factor(year), y =      avg_avg_amount, color = position_cat, group = position_cat)) +
  geom_line(plot3_data_abbott, mapping = aes(x = factor(year), y = avg_amount, group = Politician), color = "#899499", linetype = "dashed") +
  geom_text(plot3_data_no_abbott_text,
            mapping = aes(x = factor(year), y = avg_avg_amount, label = position_cat, color = position_cat), hjust = "left") + 
  geom_text(plot3_data_abbott_text,
            mapping = aes(x = factor(year), y = avg_amount, label = Politician, color = Politician), hjust = "left") + 
  scale_y_continuous(label = scales::label_dollar()) +
