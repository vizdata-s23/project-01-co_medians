...
static_list |> 
  mutate(category = case_when(hrc_pledge == TRUE & pride_or_not == TRUE ~ "both",
                              hrc_pledge == FALSE & pride_or_not == TRUE ~ "pride_not_hrc",
                              TRUE ~ "neither")) |> 
ggplot(aes(x = amount_contributed)) +
  scale_x_continuous(label = scales::label_dollar()) +
  geom_density(aes(fill = category), alpha = .5, show.legend = FALSE) +
...
ggplot(aes(x = Company, y = total_amount, 
                            pattern = hrc_pledge, fill = pride_or_not)) +
  geom_col_pattern() +
  scale_pattern_manual(values = c("none", "stripe")) +
  scale_y_continuous(label = scales::label_dollar()) +
  coord_flip() +
  labs(title = "USD Amounts of top 20 companies in contributions to anti-LGBTQ+ campaigns",
       subtitle = "Colored by ...",
       x = "Company Name",
       y = "Amount contributed to anti-LGBTQ+ campaigns (in USD)",
       fill = "Does the company donate to Pride as well?",
       pattern = "Does the company have an HRC pledge?")
```
